...
describe('googmodule', () => {
  ...
  beforeAll(() => {
    output = require.resolve(
        'build_bazel_rules_typescript_examples/googmodule/a.js');
  });

  it('should have goog module syntax in devmode', () => {
    expect(fs.readFileSync(output, {encoding: 'utf-8'}))
        .toContain(
            `goog.module('build_bazel_rules_typescript.external.build_bazel_rules_typescript_examples.googmodule.a')`);
  });
