...
func CreateFileHandler(servingPath, manifest string, pkgs []string) http.HandlerFunc {
	...
	indexHandler := func(w http.ResponseWriter, r *http.Request) {
		...
		for _, pkg := range pkgs {
			...
			userIndexFile, err := runfile(pathReplacer.Replace(filepath.Join(pkg, "index.html")))
...
func (packageNames dirHTTPFileSystem) Open(name string) (http.File, error) {
	for _, packageName := range packageNames {
		...
		realFilePath, err := rules_typescript.Runfile(filePackageName)
		...
		if err != nil {
			...
			realFilePath, err = rules_typescript.Runfile(filepath.Join(filePackageName, "index.html"))
		...
		if stat.IsDir() {
			realFilePath, err = runfile(filepath.Join(filePackageName, "index.html"))
