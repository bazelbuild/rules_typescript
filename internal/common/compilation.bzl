...
def compile_ts(ctx,
  ...
  for source in es6_sources:
    ...
    for prefix in prefixes_to_remove:
      if root[0:len(prefix)] == prefix:
        root_without_prefix = root[len(prefix):]

    file_path = "%s/%s" % (root_without_prefix, source.basename.replace(".closure", ""))
    print(file_path)
  ...
  return {
      ...
      "closure_js_library": struct(
          ...
          srcs=rerooted_es6_sources, ### js.srcs + srcs,
          ...
          js_module_roots=es6_js_modules_roots,  ### js.js_module_roots + js_module_roots,
