 ...
 */
interface BazelTsOptions extends ts.CompilerOptions {
  rootDirs: string[];
  rootDir: string;
  outDir: string;
}

function narrowTsOptions(options: ts.CompilerOptions): BazelTsOptions {
  if (!options.rootDirs) throw new Error(`compilerOptions.rootDirs should be set by tsconfig.bzl`);
  if (!options.rootDir) throw new Error(`compilerOptions.rootDirs should be set by tsconfig.bzl`);
  if (!options.outDir) throw new Error(`compilerOptions.rootDirs should be set by tsconfig.bzl`);
...
export class CompilerHost implements ts.CompilerHost, tsickle.TsickleHost {
  ...
  writeFileImpl(
    ...
    if ((this.options.module === ts.ModuleKind.AMD ||
      ...
      content = `/// <amd-module name="${sourceFiles[0].moduleName}" />\n${content}`;
    }
