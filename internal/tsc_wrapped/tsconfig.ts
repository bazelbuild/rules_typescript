...
export interface BazelOptions {
   ...
   * devmode target.
   * 
   * EXPERIMENTAL: This setting is experimental and may be removed in the future.
   */
...
export function parseTsconfig(
  ...
  if (bazelOpts.es5Mode && bazelOpts.devmodeTargetOverride) {
    switch (bazelOpts.devmodeTargetOverride.toLowerCase()) {
      case "es3": options.target = ts.ScriptTarget.ES3; break;
      case "es5": options.target = ts.ScriptTarget.ES5; break;
      case "es2015": options.target = ts.ScriptTarget.ES2015; break;
      case "es2016": options.target = ts.ScriptTarget.ES2016; break;
      case "es2017": options.target = ts.ScriptTarget.ES2017; break;
      case "es2018": options.target = ts.ScriptTarget.ES2018; break;
      case "esnext": options.target = ts.ScriptTarget.ESNext; break;
      default:
        console.error(
