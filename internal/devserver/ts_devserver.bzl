...
def _ts_devserver(ctx):
  ...
  ctx.actions.write(
      ...
      is_executable = True,
      content = """#!/bin/sh
# TODO: change to $(rootpath :devserver) after upgrading users to Bazel 0.8
RUNFILES="$PWD/.."
internal/devserver/main/main {0} \
  -base "$RUNFILES" \
  -packages={1}/{2} \
  -manifest={1}/{3}
""".format(serving_arg, workspace_name, ctx.label.package, ctx.outputs.manifest.short_path))
  return [DefaultInfo(
